...
import { CheckCircle, Clock, AlertTriangle, ShieldCheck, ShieldAlert } from 'lucide-react';
...
export default function ReferralsPage() {
  ...
  const handleOverrideStatus = async (refId: number, newStatus: 'VALID' | 'INVALID', reason?: string) => {
    setActionLoading(refId);
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/admin/referrals/${refId}/override`, {
        method: 'POST',
        headers: {
      ...
      setReferrals(
        referrals.map((r) => (r.id === refId ? { ...r, status: newStatus, failReason: updatedRef.failReason, validatedAt: updatedRef.validatedAt } : r))
      );
    } catch (err: any) {
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col gap-8">
        {/* Header */}
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Pelacakan Rujukan (Referrals)</h2>
          <p className="text-sm text-gray-400 mt-1">Pantau performa program rujukan secara real-time, audit, dan override manual jika diperlukan</p>
        </div>
        ...
        <div className="bg-[#161616] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-8 text-center text-gray-400">Belum ada aktivitas rujukan terdaftar.</div>
            ) : (
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5 text-sm">
                  {referrals.map((ref) => (
                    <tr key={ref.id} className="hover:bg-white/1 transition-all">
                      ...
                      <td className="p-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-400">
                            {ref.inviter.username ? `@${ref.inviter.username}` : `ID: ${ref.inviter.telegramId}`}
                          </span>
                        </div>
                      </td>
                      <td className="p-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-400">
                            {ref.invitee.username ? `@${ref.invitee.username}` : `ID: ${ref.invitee.telegramId}`}
                          </span>
                        </div>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-xs text-gray-400 max-w-xs truncate" title={ref.failReason || ''}>
                        {ref.failReason || '-'}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {ref.status !== 'VALID' && (
                            <button
                              ...
                              onClick={() => handleOverrideStatus(ref.id, 'VALID')}
                          ...
                          {ref.status !== 'INVALID' && (
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (reason !== null) {
                                  handleOverrideStatus(ref.id, 'INVALID', reason || 'Ditolak manual oleh Admin');
                                }
                              }}
